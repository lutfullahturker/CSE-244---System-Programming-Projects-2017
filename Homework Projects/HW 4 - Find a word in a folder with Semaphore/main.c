 ...
 *
 * Created on 25 April 2017, 14:40
 */
...
int main(int argc, char** argv) {
    ...
    if (argc != 3)
    {
        fprintf(stderr, "Wrong number of argument !\nUsage : Please enter with command line parameters like ./grepTh Word DirName\n");
        exit (1);
    }    
...
void writeSumOfFound(const char* exeDir,const char* argv1,int exitCond)
{
    FILE *countLog,*logLog,*dirTemp,*lineTemp,*dirThreadTemp;
    int i,sum = 0,temp=0,totalDir,j=0,k;
    struct timeval tEnd;
    double timedif;
...
void searchWordInFile (FILE* file,const char* search,char* argv2,const char* exeDir)
{
    ...
    FILE *logFile, *countFile,*lineCountTemp;
    int row = 1;
    int count = 0;
    int col = 1;
